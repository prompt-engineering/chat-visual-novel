...
export function replaceRouteLocale(
  pathname: string,
...
import type {
  GlobalKey as GlobalKeyEnUS,
  PageKey as PageKeyEnUS,
} from "./en-US";
import type {
  GlobalKey as GlobalKeyZhCN,
  PageKey as PageKeyZhCN,
} from "./zh-CN";

export type AppData = {
  i18n: {
    g: (key: GlobalKeyEnUS | GlobalKeyZhCN) => string;
    tFactory: <P extends PagePath>(
      path: P
    ) => (key: PageKeyEnUS<P> | PageKeyZhCN<P>) => string;
...
export async function getAppData(): Promise<AppData> {
  ...
  return dictionary().then((module) => ({
    i18n: {
      g: (key) => module["*"][key],
      tFactory: (_) => (key) =>
        (module[stripedPathname] as any)[key as any] as any,
