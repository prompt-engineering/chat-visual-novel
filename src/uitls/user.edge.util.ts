...
export async function getUser(): Promise<User | Response> {
  ...
  if (!keyHashed) {
    return new Response(
      JSON.stringify({ error: "You're not logged in yet!" }),
      {
        status: 400,
      }
    );
  ...
  if (!user) {
    return new Response(
      JSON.stringify({ error: "Your login session has been expired!" }),
      {
        status: 400,
        headers: {
          "Set-Cookie": `${SITE_USER_COOKIE}=; Max-Age=0; HttpOnly; Path=/;`,
        },
      }
    );
  }
